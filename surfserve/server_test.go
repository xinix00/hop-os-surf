...
func TestEndToEnd(t *testing.T) {
	...
	// Failover: knip de sessie door → window verdwijnt van het scherm...
	conn := <-rec.conns
	conn.Close()
	eventually(t, "window removed after connection loss", func() bool {
		_, ok := findColor(t, web.URL, red)
		return !ok
	})
	// ...en een app die blijft presenteren (zoals elke echte GUI-app) heelt
	// zichzelf: de leesgoroutine markeert de sessie dood, de eerstvolgende
	// Present herverbindt met HELLO+CREATE+vol frame. Eén frame mag verloren
	// gaan (at-most-once), het wíndow niet.
